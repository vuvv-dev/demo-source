...
export default function ProductDetailPage() {
  ...
  // Group variants by name, preserving each option's colorHex
  const variantGroups = useMemo(() => {
    if (!product?.variants?.length) return [];
    // Use Map keyed by id to preserve each distinct variant (e.g. each colorHex is a separate entry)
    const groups: Record<string, { id: string; value: string; stock: number; colorHex?: string }[]> = {};
    product.variants.forEach(v => {
      if (!groups[v.name]) groups[v.name] = [];
      groups[v.name].push({ id: v.id, value: v.value, stock: v.stock, colorHex: v.colorHex });
    });
  ...
  return (
    <main className="bg-white min-h-screen max-w-[1300px] mx-auto">
      {/* ── Sticky Nav ── */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-start">
          ...
          <div>
            ...
            <div className="relative aspect-square bg-[#f5f5f7] rounded-[28px] overflow-hidden mb-4">
              ...
              {/* Custom badge from seed metadata */}
              {!product.originalPrice && product.extraMetadata?.badge && (
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-6">
              {(product.tagline || product.shortDescription) && (
                <p className="text-[13px] font-medium text-[#bf4800] uppercase tracking-wide mb-1">
                  {product.tagline || product.shortDescription}
                </p>
              )}
              ...
              {product.shortDescription && product.tagline && product.shortDescription !== product.tagline && (
                <p className="text-[15px] text-[#86868b] mt-1">{product.shortDescription}</p>
              )}
                          ...
                          {options.map(opt => (
                            ...
                            >
                              <div
                                ...
                                style={{ backgroundColor: opt.colorHex || colorHex[opt.value] || '#ccc' }}
                              />
                            </button>
                          ))}
                        </div>
...
function AccordionItem({
  ...
  return (
    <div className="border-b border-black/5 last:border-0">
      ...
      >
        <span className="text-[24px] font-bold lg:text-xl text-[#1d1d1f] group-hover:text-[#0071e3] transition-colors">
          {title}
        </span>
        <div className={cn(
